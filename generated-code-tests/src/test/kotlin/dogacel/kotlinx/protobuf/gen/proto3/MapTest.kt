...
import testgen.maps.proto3.MapsMessage
import kotlin.test.Test
import maps.proto3.Maps

class MapTest {
    @Test
    fun shouldDeSerAll() {
        val message = Maps.MapsMessage.newBuilder()
...
//            .putAllMapSfixed64Sfixed64(mapOf(1L to 2L, 3L to 4L))
            ...
            .putAllMapStringNestedMessage(
                mapOf(
                    "1" to Maps.MapsMessage.NestedMessage.getDefaultInstance(),
                    "3" to Maps.MapsMessage.NestedMessage.getDefaultInstance()
                )
            )
            .putAllMapStringForeignMessage(
                mapOf(
                    "1" to Maps.ForeignMessage.getDefaultInstance(),
                    "3" to Maps.ForeignMessage.getDefaultInstance()
                )
            )
            .putAllMapStringNestedEnum(
                mapOf(
                    "1" to Maps.MapsMessage.NestedEnum.FOO,
                    "3" to Maps.MapsMessage.NestedEnum.BAR
                )
            )
            .putAllMapStringForeignEnum(
                mapOf(
                    "1" to Maps.ForeignEnum.FOREIGN_FOO,
                    "3" to Maps.ForeignEnum.FOREIGN_BAZ
                )
            )
            .build()
...
//        assertEquals(message.mapStringForeignMessageMap, result.mapStringForeignMessage)
        ...
        val deser = Maps.MapsMessage.parseFrom(ProtoBuf.encodeToByteArray(result))
    ...
    fun shouldDeSerEmpty() {
        val message = Maps.MapsMessage.newBuilder().build()
